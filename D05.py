...
by one, the maps describe entire ranges of numbers that can be converted. Each 
line within a map contains three numbers: the destination range start, the source 
...
## Approach
Loop over each mapper for each seed in order to calculate the result of applying the mappers
to the seeds, and then take the smallest of the resulting values
...
def part1():
    with open('D05.txt', 'r') as file:
        ...
        raw_maps = [block.split('\n') for block in blocks[1:len(blocks)]]
        maps = [[[int(num) for num in line.split(' ')] for line in map[1:len(map)]] for map in raw_maps]

        locations=[]
        for seed in seeds:
            current = seed
            for i in range(len(maps)):
                for [destination_start, source_start, r] in maps[i]:
                    if source_start <= current < source_start + r:
